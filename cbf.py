...
def read(filename, metadata=True, parse_miniheader=False):
    ...
    # Uncompress data
    if header["element_type"] == '"signed 32-bit integer"':  # DECTRIS PILATUS format
    ...
    if header["compression"] == "x-CBF_BYTE_OFFSET":
        data = uncompress(
            input_buffer,
            header["size_second_dimension"],
            header["size_fastest_dimension"],
            data_type,
        )
    elif header["compression"] == "x-CBF_NONE":
        data = numpy.frombuffer(input_buffer, dtype=data_type)
        data = data.reshape(
            (header["size_second_dimension"], header["size_fastest_dimension"])
        )
    else:
